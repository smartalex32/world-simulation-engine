...
Organizations have pack-defined purposes, roles, rules, service capacity, and
optional lifecycle policy. The initial general lifecycle forms at most one
study circle per engine-resolved settlement or community geographic scope from
bounded commons activity and evaluates bounded rotating windows for join,
role-change, and leave opportunities. Activity, proximity, recorded
relationships, curiosity, and recent co-present encounter exposure remain
separate structured factors. Selected transitions use only the named
...
Packs may also opt into an explicit leader office and recurring bounded
decision policies. Leader eligibility and decision contributions use only the
configured relationship, observer-reputation, knowledge, and persistence
weights. Candidate and participant windows are capped, stable tie-breaking is
deterministic, and pending proposals resolve with the named
`organization.decisions` RNG stream. Resolutions retain alternatives,
contributors, scores, probabilities, the roll, selected outcome, and typed
effect authorizations; organization code does not apply effects owned by other
subsystems. Membership does not create a relationship, reputation,
resource ownership, identity, belief, or automatic person-level effect.

Definitions may explicitly identify institutions, informal groups, or factions
and opt into structural evolution. Local, bounded opportunities can split a
supported dissenting subgroup, merge compatible overlapping groups, or archive
an organization below its living-member threshold. These decisions use retained
contact, relationship, and decision evidence without additional random draws.
Splits and mergers retain parent IDs and reconcile every owned unit; dissolved
records retain their closing roster and any untransferred estate assets. Archived
organizations stop providing services or executing governance. The inspector
links lineage, membership, accounts, leadership, and decision evidence to entity
history. See `docs/CONTENT_PACKS.md` for thresholds and execution bounds.
...
schema 49 / engine 0.50.0 loads directly; schema 48 / engine 0.49.0 and schema
47 / engine 0.48.0 are authenticated then migrated through their audited
behavior-upgrade boundaries; schema 46 / engine 0.47.0 and older are explicitly
...
migration-path provenance. Immutable golden fixtures are generated by
historical releases, not relabelled current state.
Migrated runs retain structural-evolution opt-out semantics and do not acquire
invented formation provenance. New runs require explicit pack evolution policy
before any structural transition can occur.

Milestone 51 also added explicit browser-worker snapshot timeout, crash, and
disposal failures, while hosted persistence ordering uses a locale-independent
binary comparator.
...
or I/O dependency. Digest-correct snapshots with structurally invalid state that
older boundaries accepted are now explicitly rejected; they are not migrated
...
needs, journeys, activities/school, decisions/actions, organization lifecycle
at its daily boundary, encounters/markets,
exposure/environment/health, monthly processing, annual processing, then daily
processing/statistics. Organization opportunities observe the current chosen
activity plus relationship/exposure evidence completed before that boundary;
same-tick encounters become input at the next lifecycle cadence. The manifest
declares each phase's cadence and named RNG stream ownership. It is not an
extension point: content packs, UI timing, and
