          ...
          <div className="placement-list">
            {value.placements.map((placement, index) => <section className="placement-card" key={placement.id} aria-label={`Placement zone ${index + 1}`}>
              ...
              {previewReady && preview && (() => {
                ...
                return <small className="placement-preview">Accepted preview: {preview.creation.populationZones.find((zone) => zone.id === placement.id)?.cellIds.length ?? 0} resolved cells · {seedPreview.eligibleHomeCellCount} home cells · {seedPreview.peoplePerHomeCell} people/home · food {seedPreview.resourceCapacityPerPerson.toFixed(2)}/person{seedPreview.averageAnchorTravelSteps === undefined ? '' : ` · ${seedPreview.averageAnchorTravelSteps} steps to anchor`}{seedPreview.recommendedPopulationCapacity === undefined ? '' : ` · profile guide ${seedPreview.recommendedPopulationCapacity.toLocaleString()}`}</small>
              })()}
            </section>)}
          </div>
