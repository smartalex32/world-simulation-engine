...
describe('authoritative population cohorts', () => {
  ...
  it('retains a hundred-thousand-person distant allocation exactly without creating detailed agents', async () => {
    const draft = {
      ...
      populationZones: [
        { id: 'west', name: 'West', preset: 'west' as const, radiusCells: 3, populationCount: 1, cohortPopulationCount: 100_000 },
    ...
    expect(cohort).toMatchObject({ id: 'cohort:west', sourceZoneId: 'west', populationCount: 100_000, householdCount: Math.ceil(100_000 / 3), eventTotals: { births: 0, deaths: 0, migrationIn: 0, migrationOut: 0 } })
