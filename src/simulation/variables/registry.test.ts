...
describe('person variable registry', () => {
  it('defines the exact nine IDs in deterministic display order', () => {
    ...
    expect(PERSON_VARIABLE_DEFINITIONS.map((definition) => definition.order)).toEqual([10, 20, 30, 40, 50, 60, 70, 80, 90])
    expect(new Set(PERSON_VARIABLE_DEFINITIONS.map((definition) => definition.id)).size).toBe(9)
  })
