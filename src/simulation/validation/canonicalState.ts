...
export function validateCanonicalSimulationState(state: SimulationState, runtime: Pick<ContentPackRuntime, 'variables' | 'organizationDefinitionById'>): void {
  ...
  if (!state.organizationLifecycle
    || !Number.isSafeInteger(state.organizationLifecycle.nextOrganizationSequence)
    || state.organizationLifecycle.nextOrganizationSequence < 1
    || !Number.isSafeInteger(state.organizationLifecycle.nextTraceSequence)
    || state.organizationLifecycle.nextTraceSequence < 1) fail('organizations', 'state.organizationLifecycle', 'lifecycle', 'Organization lifecycle state is invalid')
  requireCollection(state.organizationLifecycle.latestFormationTraces, 'organizations', 'state.organizationLifecycle.latestFormationTraces')
  requireCollection(state.organizationLifecycle.latestMembershipTraces, 'organizations', 'state.organizationLifecycle.latestMembershipTraces')
