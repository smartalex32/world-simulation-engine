...
export const WORLD_DRAFT_BUNDLE_VERSION = 2 as const
...
export function importWorldDraftBundle(value: unknown): WorldDraftRecord {
  ...
  const sourceVersion = (value as { bundleVersion?: unknown }).bundleVersion
  if (sourceVersion !== 1 && sourceVersion !== WORLD_DRAFT_BUNDLE_VERSION) throw new Error(`Unsupported draft bundle version: ${String(sourceVersion)}`)
