...
describe('world draft lifecycle domain contract', () => {
  ...
  it('preserves independently authored settlement anchors through preview and commit', async () => {
    ...
    const committedSettlements = (await SimulationEngine.create(updated.draft).snapshot()).state.world.settlements.map(({ regional: _regional, ...settlement }) => settlement)
    expect(committedSettlements).toEqual([{ id: 'northwatch', name: 'Northwatch', anchorCellId: target.id, scale: expect.any(String) }])
