...
export class SimulationEngine {
  ...
  static create(seedOrDraft: string | WorldCreationDraft, width = 32, height = 24): SimulationEngine {
    ...
    const preliminary = generateValley(draft.seed.trim() || 'valley-001', draft.width, draft.height, { terrainOverrides: draft.terrainOverrides, elevationOverrides: draft.elevationOverrides })
    const creation = normalizeWorldCreationRequest(draft, preliminary.world.grid.cells)
    const creationKey = JSON.stringify(creation)
    const { world, random } = generateValley(creation.seed, creation.width, creation.height, { name: creation.name, settlements: creation.settlements, terrainOverrides: creation.terrainOverrides, elevationOverrides: creation.elevationOverrides, idSuffix: creationKey })
