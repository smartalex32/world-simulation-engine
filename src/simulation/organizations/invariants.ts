import { validateEvolutionTraces } from './evolutionInvariants'
import type { SimulationState } from '../domain/types'
import type { OrganizationDefinition, OrganizationMembershipTrace, OrganizationTransitionTrace } from './types'
...
export function validateOrganizationState(state: SimulationState, definitions: ReadonlyMap<string, OrganizationDefinition>): void {
  ...
  const ids = new Set(organizationsById.keys())
  ...
  for (const organization of state.organizations) {
    ...
    if (state.config.organizationEvolutionModelVersion === 1 && (!organization.specialization || !organization.status || !organization.lineage)) fail('organizations', `state.organizations.${organization.id}`, 'structural-identity', 'Organization structural identity and lineage are required')
    if (organization.specialization !== undefined && organization.specialization !== (definition.specialization ?? 'institution')) fail('organizations', `state.organizations.${organization.id}.specialization`, 'specialization', `Organization ${organization.id} has incompatible specialization`)
    if (organization.status !== undefined && organization.status !== 'active' && organization.status !== 'dissolved') fail('organizations', `state.organizations.${organization.id}.status`, 'status', `Organization ${organization.id} has invalid status`)
    if (organization.lineage !== undefined && (!['initial', 'formation', 'schism', 'merger'].includes(organization.lineage.origin) || !Number.isSafeInteger(organization.lineage.formedTick) || organization.lineage.formedTick < 0 || organization.lineage.formedTick > state.tick || organization.lineage.parentOrganizationIds.some((id, index) => !ids.has(id) || id === organization.id || (index > 0 && organization.lineage!.parentOrganizationIds[index - 1]! >= id)))) fail('organizations', `state.organizations.${organization.id}.lineage`, 'lineage', `Organization ${organization.id} has invalid lineage`)
    if (organization.lineage) {
      const lineage = organization.lineage
      const expectedParents = lineage.origin === 'merger' ? 2 : lineage.origin === 'schism' ? 1 : 0
      if (lineage.parentOrganizationIds.length !== expectedParents || (lineage.origin === 'initial' && lineage.formedTick !== 0) || lineage.parentOrganizationIds.some((id) => (organizationsById.get(id)?.lineage?.formedTick ?? -1) >= lineage.formedTick)) fail('organizations', `state.organizations.${organization.id}.lineage`, 'lineage-origin', 'Organization lineage has invalid parents or formation order')
    }
  ...
  const transitionTraces = lifecycle.latestTransitionTraces ?? []
  ...
  if (lifecycle.latestFormationTraces.length > ORGANIZATION_LIFECYCLE_TRACE_LIMIT || lifecycle.latestMembershipTraces.length > ORGANIZATION_LIFECYCLE_TRACE_LIMIT || transitionTraces.length > ORGANIZATION_LIFECYCLE_TRACE_LIMIT) fail('organizations', 'state.organizationLifecycle', 'trace-limit', 'Organization lifecycle trace history exceeds its bound')
  ...
  const traceSequences = [...lifecycle.latestFormationTraces, ...lifecycle.latestMembershipTraces, ...transitionTraces].map((trace) => trace.sequence)
  ...
  if (state.config.organizationEvolutionModelVersion === 1 && [...definitions.values()].some((definition) => definition.lifecycle?.evolution)) {
    const ordered = [...traceSequences].sort((a, b) => a - b)
    const expectedCount = Math.min(ORGANIZATION_LIFECYCLE_TRACE_LIMIT, lifecycle.nextTraceSequence - 1)
    if (ordered.length !== expectedCount || ordered.some((sequence, index) => sequence !== lifecycle.nextTraceSequence - expectedCount + index)) fail('organizations', 'state.organizationLifecycle', 'trace-suffix', 'Organization lifecycle evidence is not a contiguous retained suffix')
  }
  ...
  validateEvolutionTraces(state, definitions)
  validateMembershipHistory(state.organizations, lifecycle.latestMembershipTraces, transitionTraces)
}
...
function validateMembershipHistory(organizations: SimulationState['organizations'], traces: readonly OrganizationMembershipTrace[], structural: readonly OrganizationTransitionTrace[]): void {
  ...
  for (const trace of [...traces, ...structural].sort((first, second) => second.sequence - first.sequence)) {
    if ('membershipsAfter' in trace) {
      for (const entry of trace.membershipsAfter) {
        const current = rolesByOrganization.get(entry.organizationId)
        if (!current || current.size !== entry.members.length || entry.members.some((member) => current.get(member.personId) !== member.role)) fail('organizations', 'state.organizationLifecycle.latestTransitionTraces', 'transition-history', 'Structural membership history does not match its after state')
      }
      for (const entry of trace.membershipsAfter) rolesByOrganization.set(entry.organizationId, new Map())
      for (const entry of trace.membershipsBefore) rolesByOrganization.set(entry.organizationId, new Map(entry.members.map((member) => [member.personId, member.role])))
      continue
    }
