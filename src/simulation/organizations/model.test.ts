...
describe('organizations', () => {
  it('creates persistent schools with explicit learner memberships only', () => {
    ...
    const result = createInitialSchools(people as never, ['1,1'], DEFAULT_PREINDUSTRIAL_PACK.organizationDefinitions[0]!)
    expect(result[0]).toMatchObject({ kind: 'school', locationCellId: '1,1', serviceCapacity: 24, members: [{ personId: 'child', role: 'learner' }] })
  })
})
