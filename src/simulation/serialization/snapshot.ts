...
export async function validateSnapshot(value: unknown): Promise<SnapshotEnvelope> {
  ...
  if (snapshot.state.world.name !== normalizedCreation.name || canonicalStringify(snapshot.state.world.settlements) !== canonicalStringify(normalizedCreation.settlements)) {
