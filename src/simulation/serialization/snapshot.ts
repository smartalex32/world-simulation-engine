...
export async function validateSnapshot(value: unknown, contentPack: ContentPack | ResolvedContentPack = DEFAULT_PREINDUSTRIAL_PACK): Promise<SnapshotEnvelope> {
  ...
  if (snapshot.state.config.organizationLeadershipDecisionModelVersion !== 0 && snapshot.state.config.organizationLeadershipDecisionModelVersion !== ORGANIZATION_LEADERSHIP_DECISION_MODEL_VERSION) throw new Error('Unsupported organization leadership/decision configuration')
  if (snapshot.state.config.organizationEvolutionModelVersion !== 0 && snapshot.state.config.organizationEvolutionModelVersion !== ORGANIZATION_EVOLUTION_MODEL_VERSION) throw new Error('Unsupported organization evolution configuration')
