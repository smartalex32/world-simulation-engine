...
describe('organization projection', () => {
  ...
  it('projects detached owned balances, transfers, and observer-specific evidence', () => {
    ...
    const proposal = { sequence: 1, policyId: 'policy.study', proposedTick: 4, resolvesAtTick: 28, participantIds: ['person-1'], participantRoles: [{ personId: 'person-1', memberRoleId: 'member' }], alternatives: ['alternative.a', 'alternative.b'] }
    const profiles = buildProjectedOrganizationProfiles([{ id: 'circle-1', name: 'Circle', kind: 'circle', locationCellId: '0,0', activityLocationId: 'activity.commons.0,0', members: [], serviceCapacity: 4, sharedRuleIds: [], assets: { currencyUnits: 3, goods: { 'good.food': 1 }, latestTransferTraces: [transfer] }, reputationLedger: { nextObservationSequence: 2, observations: [observation], currentByObserver: [{ observer: observation.observer, valuePermille: 510, lastObservationSequence: 1, lastObservedTick: 4 }] }, leadership: { nextTraceSequence: 1, leaderPersonId: 'person-1', roleId: 'steward', termStartedTick: 4, latestTraces: [] }, decisions: { nextProposalSequence: 2, pending: [proposal], latestResolutions: [] } }], [], [{ id: 'circle', name: 'Circle', purposeIds: [], memberRoleIds: [], sharedRuleIds: [], initialService: { location: 'settlement-anchor', activityLocation: 'commons', serviceCapacity: 4 }, assets: { initialCurrencyUnits: 3, initialGoods: { 'good.food': 2 } }, reputation: { enabled: true } }])
    expect(profiles[0]).toMatchObject({ ownedResourcesStatus: 'owned-account', ownedCurrencyUnits: 3, ownedGoods: { 'good.food': 1 }, reputationStatus: 'observer-evidence', latestAssetTransferEvidence: [transfer], latestReputationEvidence: [observation], reputationByObserver: [{ observer: observation.observer, valuePermille: 510 }], leadershipStatus: 'filled', leaderPersonId: 'person-1', leaderRoleId: 'steward', decisionStatus: 'active', pendingDecisions: [proposal] })
    profiles[0]!.ownedGoods!['good.food'] = 99
    profiles[0]!.latestReputationEvidence![0]!.valuePermille = 0
    profiles[0]!.pendingDecisions![0]!.participantIds.push('mutated')
    expect(transfer.nextFromAmount).toBe(1)
    expect(observation.valuePermille).toBe(510)
    expect(proposal.participantIds).toEqual(['person-1'])
  })
})
