...
import type { OrganizationSpecialization, OrganizationTransitionTrace, OrganizationAssetTransferTrace, OrganizationDecisionProposal, OrganizationDecisionResolution, OrganizationLeadershipTrace, OrganizationMembershipTrace, OrganizationReputationCurrent, OrganizationReputationObservation } from '../simulation/organizations/types'

/** Incremented when the bounded worker-to-workbench projection shape changes. */
export const PROJECTION_PROTOCOL_VERSION = 20
...
export interface ProjectedOrganizationProfile { structuralLimitReason?: 'member-limit' | 'goods-limit'; specialization?: OrganizationSpecialization; status?: 'active' | 'dissolved'; lineage?: OrganizationState['lineage']; latestTransitionEvidence?: OrganizationTransitionTrace[]; closedMembership?: OrganizationState['closedMembership']; id: string; name: string; kind: OrganizationState['kind']; definitionName: string; purposeIds: string[]; allowedMemberRoleIds: string[]; locationCellId: string; goal: 'education' | 'unspecified'; memberCount: number; roleCounts: Record<string, number>; serviceCapacity: number; sharedRuleIds: string[]; lifecycleStatus: 'disabled' | 'formation-enabled' | 'membership-only'; latestMembershipEvidence?: OrganizationMembershipTrace; internalRelationshipCount: number; internalAverageFamiliarity: number; reputationStatus: 'not-measured' | 'observer-evidence'; ownedResourcesStatus: 'not-modeled' | 'owned-account'; ownedCurrencyUnits?: number; ownedGoods?: Record<string, number>; latestAssetTransferEvidence?: OrganizationAssetTransferTrace[]; latestReputationEvidence?: OrganizationReputationObservation[]; reputationByObserver?: OrganizationReputationCurrent[]; leadershipStatus: 'not-modeled' | 'vacant' | 'filled'; leaderPersonId?: string; leaderRoleId?: string; leadershipTermStartedTick?: number; latestLeadershipEvidence?: OrganizationLeadershipTrace[]; decisionStatus: 'not-modeled' | 'active'; pendingDecisions?: OrganizationDecisionProposal[]; latestDecisionResolutions?: OrganizationDecisionResolution[] }
...
export interface ProjectedEconomicSummary { householdCount: number; householdsWithoutFoodCount: number; foodUnits: number; toolUnits: number; foodGiniPermille: number; toolGiniPermille: number; currencyUnits: number; currencyGiniPermille: number; goodsById: Record<string, number>; marketPrices: { marketId: string; prices: Record<string, number>; treasuryUnits: number }[]; totalTaxCollectedUnits: number; retainedTradeCount: number; retainedProductionCount: number; retainedWageCount: number; occupationCounts: { dependent: number; household: number; forager: number; unassigned: number } }
...
export interface WorkbenchProjection {
  ...
  effectiveConfiguration: import('../simulation/projection/types').WorldProjection['effectiveConfiguration']
  phaseManifest: import('../simulation/projection/types').WorldProjection['phaseManifest']
