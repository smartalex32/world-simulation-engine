...
import { OrganizationInspector } from './ui/organizations/OrganizationInspector'
...
export default function App() {
  ...
  /** RUN_CREATED remains in bounded frames; reset persistence once per actual creation lifecycle. */
  const runCreationResetPending = useRef(true)
  /** Invalidates checkpoints that began before a new authoritative run commit. */
  ...
  async function refreshContentPacks() { try { setContentPacks(await database.listContentPacks()) } catch (reason) { setError(`Content packs failed: ${messageOf(reason)}`) } }
  async function saveContentPackJson() { try { const pack = importContentPack(contentPackJson); await database.saveContentPack(pack); await refreshContentPacks(); setContentPackJson(exportContentPack(pack)) } catch (reason) { setError(`Content pack is invalid: ${messageOf(reason)}`) } }
  ...
  useEffect(() => {
    ...
    async function handleResponse(response: SimulationResponse) {
      if (response.type === 'FRAME') {
        ...
        if (runCreationResetPending.current && response.events.some((event) => event.type === 'RUN_CREATED')) {
          runCreationResetPending.current = false
          const creationEpoch = telemetryEpoch.current
          try {
            await persistenceController.requestSnapshot(async () => {
              await database.resetRunTelemetry(response.projection.runId)
              if (telemetryEpoch.current !== creationEpoch) return
    ...
    void refreshSnapshots(); void refreshContentPacks()
  ...
  async function autosave() {
    ...
    const checkpointEpoch = telemetryEpoch.current
    try {
      const snapshot = await persistenceController.requestSnapshot(async () => {
        if (telemetryEpoch.current !== checkpointEpoch) return undefined
      ...
      if (telemetryEpoch.current === checkpointEpoch) setError(`Autosave failed: ${messageOf(reason)}`)
    }
  }
        ...
        right={<>
          ...
          : selectedCommunity
            ...
            : selectedOrganization
              ? <OrganizationInspector organization={selectedOrganization} onInspectPerson={(personId) => navigation.selectEntity({ kind: 'person', id: personId }, { focus: true, workspace: 'entities', detailSurface: 'inspector' })} onInspectOrganization={(organizationId) => navigation.selectEntity({ kind: 'organization', id: organizationId })} onShowMap={(cellId) => navigation.selectEntity({ kind: 'map-cell', id: cellId }, { focus: true, workspace: 'world', detailSurface: 'map' })} onShowHistory={() => navigation.selectEntity({ kind: 'organization', id: selectedOrganization.id }, { workspace: 'history', detailSurface: 'timeline' })} />
...
function worldSetupFromCreation(creation: WorldCreationRequest): WorldSetupValues {
  return {
    ...
    placements: creation.populationZones.map((zone, index) => {
      ...
      return { id: zone.id, name: zone.name, region, preset: region, radiusCells: 3, allocation: zone.populationCount, cohortAllocation: zone.cohortPopulationCount ?? 0, settlementId: zone.settlementId, settlementName: settlement?.name, template: zone.template, cellIds: [...zone.cellIds] }
    }),
...
function creationDraftFromSetup(setup: WorldSetupValues): WorldCreationDraft {
  return {
    ...
    settlements: setup.settlements.map((settlement) => ({ ...settlement, ...(settlement.catchmentCellIds === undefined ? {} : { catchmentCellIds: [...settlement.catchmentCellIds] }) })),
    ...
    populationZones: setup.placements.map((placement) => ({
      ...
      ...(placement.cohortAllocation === undefined || placement.cohortAllocation === 0 ? {} : { cohortPopulationCount: placement.cohortAllocation }),
...
function worldSetupFromDraft(draft: WorldCreationDraft): WorldSetupValues {
  return {
    ...
    placements: draft.populationZones.map((zone, index) => {
      ...
      return { id: zone.id, name: zone.name, region: regionForPreset(preset), preset, radiusCells: zone.radiusCells ?? 3, allocation: zone.populationCount, cohortAllocation: zone.cohortPopulationCount ?? 0, settlementId: zone.settlementId, settlementName: settlement?.name, template: zone.template, ...(zone.cellIds !== undefined ? { cellIds: [...zone.cellIds] } : {}) }
    }),
