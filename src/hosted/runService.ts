...
export class HostedRunService {
  ...
  async advanceJob(ownerToken: string, expected: HostedRunObservation, count: number, jobFor?: (after: HostedRunObservation) => HostedSimulationJob): Promise<HostedRunObservation> { this.authorize(ownerToken); if (!Number.isSafeInteger(count) || count < 1) throw new Error('Hosted job step count must be a positive safe integer'); return this.serial(async () => { const before = await this.snapshot(this.engine); if (before.state.tick !== expected.tick || before.digest !== expected.digest) throw new Error('Hosted job run state conflict'); const candidate = await SimulationEngine.restore(before); const result = candidate.advance(count); const after = await this.snapshot(candidate); const observation = { tick: after.state.tick, digest: after.digest }; const job = jobFor?.(observation); const id = `job:${job ? `${job.jobId}:${expected.digest}` : `${expected.digest}:${count}`}`; const outcome = await this.commit(before, after, id, createHash('sha256').update(id).digest('hex'), result.events, result.statistics, job); this.replaceEngine(outcome === 'already-committed' ? await this.restoreDurable() : candidate); return outcome === 'already-committed' ? await this.observeUnqueued() : observation }) }
  ...
  private async serial<T>(operation: () => Promise<T>): Promise<T> { let result!: T; const queued = this.commandQueue.then(async () => { try { result = await operation() } catch (error) { this.replaceEngine(await this.restoreDurable()); throw error } }); this.commandQueue = queued.then(() => undefined, () => undefined); await queued; return result }
