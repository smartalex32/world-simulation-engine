...
export class HostedRunService {
  ...
  async advanceJob(ownerToken: string, expected: HostedRunObservation, count: number, jobFor?: (after: HostedRunObservation) => HostedSimulationJob): Promise<HostedRunObservation> { this.authorize(ownerToken); if (!Number.isSafeInteger(count) || count < 1) throw new Error('Hosted job step count must be a positive safe integer'); return this.serial(async () => { const before = await this.snapshot(this.engine); if (before.state.tick !== expected.tick || before.digest !== expected.digest) throw new Error('Hosted job run state conflict'); const candidate = await SimulationEngine.restore(before); const result = candidate.advance(count); const after = await this.snapshot(candidate); const observation = { tick: after.state.tick, digest: after.digest }; const job = jobFor?.(observation); const id = `job:${job ? `${job.jobId}:${expected.digest}` : `${expected.digest}:${count}`}`; const fingerprint = createHash('sha256').update(canonicalStringify({ id, count, expected, job })).digest('hex'); const outcome = await this.commit(before, after, id, fingerprint, result.events, result.statistics, job); this.replaceEngine(outcome === 'already-committed' ? await this.restoreDurable() : candidate); return outcome === 'already-committed' ? await this.observeUnqueued() : observation }) }
  private async apply(command: HostedRunCommand): Promise<HostedCommandResult> {
    ...
    const after = await this.snapshot(candidate); const outcome = await this.commit(before, after, `command:${command.requestId}`, fingerprint(command), events, statistics); this.replaceEngine(outcome === 'already-committed' ? await this.restoreDurable() : candidate, command.type === 'RESET')
    const responses: SimulationResponse[] = [this.frame(command.requestId, outcome === 'already-committed' ? [] : events, outcome === 'already-committed' ? [] : statistics)]; if (command.type === 'STEP' || command.type === 'RESET') responses.push({ type: 'STATUS', requestId: command.requestId, status: 'paused', ticksPerBatch: 1 }); return this.result(responses)
  }
  ...
  private replaceEngine(engine: SimulationEngine, resetViewport = false): void { this.engine = engine; const source = engine.project(); this.projectionBuilder = new WorkbenchProjectionBuilder(source); if (resetViewport) this.viewport = defaultViewport(source.world.grid.width, source.world.grid.height) }
  ...
  private async commit(before: WorkbenchSnapshotEnvelope, after: WorkbenchSnapshotEnvelope, mutationId: string, mutationFingerprint: string, events: readonly SimulationEvent[], statistics: readonly StatisticSample[], job?: HostedSimulationJob): Promise<'committed' | 'already-committed'> { const record = this.record(after); if (!isMutationStore(this.store)) throw new Error('Hosted authoritative storage must support transactional mutations'); return this.store.commitRunMutation({ expectedTick: before.state.tick, expectedDigest: before.digest, mutationId, mutationFingerprint, record, events, statistics, job }) }
  ...
  private async serial<T>(operation: () => Promise<T>): Promise<T> { this.assertReconciled(); let result!: T; const queued = this.commandQueue.then(async () => { try { result = await operation() } catch (error) { try { this.replaceEngine(await this.restoreDurable()) } catch { this.unreconciled = true } throw error } }); this.commandQueue = queued.then(() => undefined, () => undefined); await queued; return result }
