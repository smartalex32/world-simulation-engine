...
export class HostedRunService {
  private readonly application = new SimulationApplicationService()
  ...
  private async apply(command: HostedRunCommand, sharedWorld?: Omit<SharedWorldCommitRequest, 'initialRun'>): Promise<HostedTransactionalCommandResult> {
    if (command.type === 'SET_VIEWPORT') { this.viewport = normalizeViewport(command.viewport); return { result: this.result([this.frame(command.requestId)], command) } }
    if (command.type === 'REQUEST_SNAPSHOT') { const snapshot = await this.snapshot(this.engine); return { result: this.result([{ type: 'SNAPSHOT', requestId: command.requestId, snapshot }, this.frame(command.requestId)], command) } }
    if (command.type === 'PAUSE') return { result: this.result([{ type: 'STATUS', requestId: command.requestId, status: 'paused', ticksPerBatch: 1 }], command) }
    if (command.type === 'SET_SPEED') return { result: this.result([{ type: 'STATUS', requestId: command.requestId, status: 'paused', ticksPerBatch: boundedSpeed(command.ticksPerBatch) }], command) }
    ...
    if (command.type === 'STEP' || command.type === 'MATERIALIZE_COHORT' || command.type === 'DEMATERIALIZE_PEOPLE' || command.type === 'SET_PROTECTED_PEOPLE') { const execution = this.application.execute(command, { engine: candidate }); events.push(...execution.events); statistics.push(...execution.statistics); projectionInvalidation = execution.projectionInvalidation }
