...
async function showPopulationStep(setup: import('@playwright/test').Locator): Promise<void> {
  await setup.getByRole('button', { name: 'Next: Population', exact: true }).click()
  await expect(setup.getByRole('button', { name: 'Back to map', exact: true })).toBeVisible()
}
...
capabilityTest('authoring', 'opens the world setup surface with explicit scale and placement allocations', async ({ page }) => {
  ...
  await showPopulationStep(setup)
  ...
  await expect(setup.getByRole('button', { name: 'Commit world', exact: true })).toBeEnabled()
  await expect(page.getByText('Communities are geographic exposure measures, not memberships.')).toBeVisible()
  await setup.getByRole('button', { name: 'Commit world', exact: true }).click()
...
capabilityTest('authoring', 'opens build and population drafts from the topbar while preserving staged draft edits', async ({ page }) => {
  await page.goto('/')
  await page.getByRole('button', { name: 'Build', exact: true }).click()
  const setup = page.getByRole('dialog', { name: 'Shape a new world' })
  await expect(setup.getByRole('button', { name: 'Terrain', exact: true })).toBeVisible()
  await setup.getByLabel('World name').fill('Topbar Valley')
  await showPopulationStep(setup)
  await setup.getByLabel('Zone 1 people').fill('101')
  await setup.getByRole('button', { name: 'Back to map', exact: true }).click()
  await expect(setup.getByLabel('World name')).toHaveValue('Topbar Valley')
...
capabilityTest('authoring', 'authors a deterministic blank-land canvas before committing a world', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Create world', exact: true }).click()
  const setup = page.getByRole('dialog', { name: 'Shape a new world' })
  await expect(setup.getByRole('button', { name: 'Next: Population', exact: true })).toBeEnabled()
  ...
  await expect(setup.getByLabel('Terrain paint type')).toBeVisible()
...
capabilityTest('authoring', 'persists an accepted drawn zone through reload and commits its authoritative cells', async ({ page }) => {
  ...
  await expect.poll(async () => map.getAttribute('data-draft-map-first-passable-center')).not.toBeNull()
  const selection = setup.locator('.draft-zone-map-heading small')
  const initialSelection = await selection.textContent()
  const initialSelectedCount = Number(initialSelection?.match(/\d+/)?.[0] ?? Number.NaN)
  expect(Number.isFinite(initialSelectedCount)).toBe(true)
  const [x, y] = (await map.getAttribute('data-draft-map-first-passable-center'))!.split(',').map(Number)
  const passablePixel = { x: x!, y: y! }
  await map.click({ position: passablePixel })
  await expect.poll(async () => Number((await selection.textContent())?.match(/\d+/)?.[0] ?? Number.NaN)).not.toBe(initialSelectedCount)
  const selectedAfterClick = Number((await selection.textContent())?.match(/\d+/)?.[0] ?? Number.NaN)
  expect(Math.abs(selectedAfterClick - initialSelectedCount)).toBe(1)
  await setup.getByRole('button', { name: 'Apply drawn cells', exact: true }).click()
  const acceptedMeta = setup.locator('.placement-card').first().locator('.placement-meta')
  await expect(acceptedMeta).toContainText(/Resolved cells: \d+/)
  const acceptedMetaText = await acceptedMeta.textContent()

  await page.reload()
  await page.getByRole('button', { name: 'Create world', exact: true }).click()
  setup = page.getByRole('dialog', { name: 'Shape a new world' })
  await showPopulationStep(setup)
  const reloadedMeta = setup.locator('.placement-card').first().locator('.placement-meta')
  await expect(reloadedMeta).toContainText(/Resolved cells: \d+/)
  await expect(reloadedMeta).toHaveText(acceptedMetaText ?? '')
  map = setup.getByLabel('Draft placement zone map')
  await expect(setup.locator('.draft-zone-map-heading small')).toContainText(`${selectedAfterClick} selected`)
  await setup.getByRole('button', { name: 'Commit world', exact: true }).click()
  await expect(setup).toBeHidden()
  await expect(page.locator('.world-overview strong')).toHaveText('The Seeded Valley')
})
...
capabilityTest('authoring', 'authors arbitrary stable placement zones before a draft commit', async ({ page }) => {
  ...
  await showPopulationStep(restored)
  ...
  await expect(restored.getByRole('button', { name: 'Commit world', exact: true })).toBeDisabled()
...
capabilityTest('reproducibility', '@critical creates, steps, inspects, and saves a deterministic world', async ({ page }) => {
  ...
  await openRecordedPersonDetails(page)
  await expect(page.getByText('Person inspector')).toBeVisible()
  await expect(page.getByText('Person hooked')).toBeVisible()
...
capabilityTest('inspection', 'shows a compact person overview before recorded details are expanded', async ({ page }) => {
  await page.goto('/?entity=person%3Aperson-0001&focus=person%3Aperson-0001')
  const overview = page.getByRole('region', { name: 'Person overview' })
  await expect(overview).toBeVisible()
  await expect(overview.getByRole('heading', { name: 'person-0001', exact: true })).toBeVisible()
  await expect(overview).toContainText(/\d+ years/)
  await expect(overview.locator('meter').first()).toBeVisible()
  const details = overview.locator('details.recorded-person-details')
  await expect(details).not.toHaveAttribute('open', '')

  await overview.getByRole('button', { name: 'Relationships', exact: true }).click()
  await expect(page).toHaveURL(/workspace=entities/)
  await expect(page.getByRole('heading', { name: /Relationships around person-0001/ })).toBeVisible()

  await page.goto('/?entity=person%3Aperson-0001&focus=person%3Aperson-0001')
  await page.getByRole('region', { name: 'Person overview' }).getByRole('button', { name: 'Life history', exact: true }).click()
  await expect(page).toHaveURL(/workspace=history/)
  await expect(page.getByRole('heading', { name: 'Multi-lane recorded evidence' })).toBeVisible()
})
...
capabilityTest('quality', '@critical follows persisted causal evidence from controls through history, geography, and analytics', async ({ page }, testInfo) => {
  ...
  await page.goto('/?workspace=simulation')
  ...
  await page.locator('.save-form').getByRole('button', { name: 'Save', exact: true }).click()
  await expect(page.getByText('Saved snapshot: Convergence evidence', { exact: true })).toBeVisible({ timeout: 90_000 })

  const currentCommunityEvent = page.locator('.event-row').filter({ hasText: 'COMMUNITY MEASURES UPDATED' }).first()
  await expect(currentCommunityEvent).toBeVisible()
  await currentCommunityEvent.getByRole('link', { name: 'Open event' }).click()
  await expect(page.getByRole('heading', { name: 'Multi-lane recorded evidence' })).toBeVisible()
  await expect(page.getByRole('button', { name: 'Refresh history' })).toBeVisible({ timeout: 90_000 })
  ...
  await expect(page.getByRole('heading', { name: 'Seeded Valley' })).toBeVisible()
  await expect(page.getByRole('heading', { name: 'Emergent measures' })).toBeVisible()
...
async function openRecordedPersonDetails(page: import('@playwright/test').Page): Promise<void> {
  const details = page.locator('details.recorded-person-details').last()
